...
def build_expansion_title_card(
    title_text: str = t("title"),
    image_path: str = "deepfield.png",
    ...
    if image_path and os.path.exists(image_path):
        img = mn.ImageMobject(image_path).scale(0.4)
...
def show_expansion_title_card(
    ...
    group = build_expansion_title_card(title_text, image_path, logo_path)
    scene.add(group)
    scene.wait(hold)
...
class TitleCard(mn.Scene):
    def construct(self):
        ...
        group = build_expansion_title_card(title_text=t("title"),
