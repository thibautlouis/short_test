...
# Try to load i18n helper if present
try:
    from drake_lang import t
except Exception:
    def t(key: str):
        # Fallback: simple keys if drake_lang.py isn't available
        defaults = {
            "title": "default",
            "equation": r"N = R_\star \, f_p \, n_e \, f_l \, f_i \, f_c \, L",
        }
...
# ---------- Test ----------
class DrakeTitleCard(mn.Scene):
    def construct(self):
        self.camera.background_color = BG
        lang = os.getenv("SHORT_LANG", "FR")
        
        g = build_title_card_group(title_text=t("short_title"),
                                  formula_tex=("math", r""),
                                  kepler_path="galaxy.png",
